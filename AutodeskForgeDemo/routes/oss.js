...
// Middleware for obtaining a token for each request.用于为每个请求获取令牌的中间件。
...
// returns a JSON with list of buckets, otherwise returns a JSON with list of objects in bucket with given name.返回带有存储桶列表的JSON，否则返回带有给定名称的存储桶中的对象列表的JSON。
router.get('/buckets', async (req, res, next) => {
  const bucket_name = req.query.id;
  if (!bucket_name || bucket_name === '#') {
    try {
      // Retrieve buckets from Forge using the [BucketsApi]使用[BucketsApi]从Forge检索存储桶(https://github.com/Autodesk-Forge/forge-api-nodejs-client/blob/master/docs/BucketsApi.md#getBuckets)
      ...
      res.json(buckets.body.items.map((bucket) => {
        return {
          id: bucket.bucketKey,
          // Remove bucket key prefix that was added during bucket creation删除在创建存储桶期间添加的存储桶键前缀
